...
web: python manage.py migrate --noinput && python manage.py seed_users && python manage.py seed_centers && python manage.py seed_demo_mpdsr && python manage.py seed_demo_phd_bandhu && gunicorn spondon.wsgi --bind 0.0.0.0:$PORT --workers 2 --timeout 120
