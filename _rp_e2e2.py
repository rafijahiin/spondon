...
# 1. register ONE action with a WORKER-TYPED id (the new fistula-style flow)
plan = {'_id': 999999200, 'ap_mode': 'new_plan', 'grp_meta/district': 'kurigram',
        'grp_meta/collection_date': '2026-06-25', '_submitted_by': 'wiring_test',
        f'{G}/action_id': AID, f'{G}/rp_section': 'system_strengthening',
        f'{G}/act_activity': 'WIRING TEST fistula-style — safe to delete',
        f'{G}/act_status': 'pending'}
r1 = requests.post(EP, headers=HK, data=json.dumps(plan), timeout=60)
print('1) register %s   ->' % AID, r1.status_code, repr(r1.text[:46]))

time.sleep(15)   # let the post_save signal sync mpdsr_actions.csv

# 2. confirm the typed id is now in the "update an action" dropdown CSV
...
# 3. update it by id (proves "update later"), set dropped to clean up
upd = {'_id': 999999201, 'ap_mode': 'update_action', 'grp_meta/district': 'kurigram',
       'grp_update/ap_action_sel': AID.lower(), 'grp_update/ap_new_status': 'dropped',
