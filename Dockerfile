...
CMD ["sh", "-c", "python manage.py migrate --noinput && python manage.py seed_users && python manage.py seed_centers && python manage.py backfill_f4_facility && if [ \"$PURGE_PHD_DATA\" = \"1\" ]; then python manage.py purge_phd_data --confirm; fi && if [ \"$PRUNE_PHD_CENTRES\" = \"1\" ]; then python manage.py prune_phd_centres --confirm; fi && if [ \"$REFRESH_DEMO_SEED\" = \"1\" ]; then python manage.py seed_demo_mpdsr --purge && python manage.py seed_demo_fistula --purge && python manage.py seed_demo_phd_bandhu --purge && python manage.py seed_demo_mpdsr && python manage.py seed_demo_phd_bandhu && python manage.py seed_demo_fistula && python manage.py backfill_worker_name && python manage.py backfill_f4_facility; fi && if [ \"$FLUSH_PRACTICE_DATA\" = \"1\" ]; then python manage.py flush_practice_data --confirm; fi && gunicorn spondon.wsgi --bind 0.0.0.0:$PORT --workers 1 --threads 4 --timeout 120"]
